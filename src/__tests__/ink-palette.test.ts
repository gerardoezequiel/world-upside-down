...
describe('buildTextPairings', () => {
  ...
  it('each pairing has valid color, shadow, and name', () => {
    const pairings = buildTextPairings(classicPalette);
  ...
  it('deduplicates inks (mono palette has fewer pairings)', () => {
    ...
    // mono: water=blue, built=blue, green=blue → only 3 unique inks
...
describe('buildDerivedPalette', () => {
  ...
  it('labelHalo is always white', () => {
    expect(buildDerivedPalette(classicPalette).labelHalo).toBe('#FFFFFF');
  });
  ...
  it('road, rail, boundary, label all use ink hex', () => {
    const derived = buildDerivedPalette(classicPalette);
  ...
  it('earth is very light (close to white)', () => {
    const derived = buildDerivedPalette(classicPalette);
    const earthR = parseInt(derived.earth.slice(1, 3), 16);
    expect(earthR).toBeGreaterThan(200);
  });
  ...
  it('produces different outputs for different palettes', () => {
    ...
    expect(classic.park).not.toBe(protest.park);
  });

  it('works for all 6 preset palettes', () => {
...
describe('INK_CATALOG integrity', () => {
  ...
  it('every ink has id matching its key, name, and valid hex', () => {

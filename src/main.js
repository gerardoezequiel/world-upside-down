...
function recolorStyle(style) {
    ...
    for (const layer of s.layers) {
        ...
        if (id === "buildings" && "paint" in layer) {
            ...
            layer.paint["fill-outline-color"] = "rgba(0, 120, 191, 0.25)";
            continue;
        }
        // Water fill
        ...
        if (id === "landuse_urban_green" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.parkAlt;
            continue;
        }
        // Hospital, school, industrial, etc.
        if (id === "landuse_hospital" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.hospital;
            continue;
        }
        if (id === "landuse_school" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.school;
            continue;
        }
        if (id === "landuse_industrial" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.industrial;
            continue;
        }
        if (id === "landuse_beach" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.beach;
            continue;
        }
        if (id === "landuse_zoo" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.zoo;
            continue;
        }
        if (id === "landuse_aerodrome" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.aerodrome;
            continue;
        }
        ...
        if (id === "landuse_pier" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.pier;
            continue;
        }
        if (id === "landuse_runway" && "paint" in layer) {
            layer.paint["fill-color"] = PALETTE.runway;
            continue;
        }
        // Landcover (low zoom)
        ...
        if (id.startsWith("boundaries") && "paint" in layer) {
            layer.paint["line-color"] = PALETTE.boundary;
            continue;
        }
...
function updateScaleBar(map) {
    ...
    for (let m = 0; m <= cfg.detail; m += cfg.dStep) {
        ...
        if (m === 0)
            ...
            label = useKm ? String(m / 1000) : String(m);
...
function updateCityTitle(map) {
    ...
    geocodeTimeout = setTimeout(async () => {
        ...
        try {
            const res = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${ctr.lat}&lon=${ctr.lng}&zoom=${Math.min(Math.round(z), 14)}&format=json&accept-language=en`, { headers: { 'User-Agent': 'world-upside-down/1.0' } });
...
function applyOrientation(map, target) {
    ...
    const needsBearingChange = (prev === 'upside-down' && target !== 'upside-down') ||
    ...
    if (title) {
        ...
        else if (target === 'mirrored')
            title.classList.add('mirrored');
        // default (upside-down) has no extra class → CSS rotates 180
    }
    // Subtitle
    if (sub) {
        if (target === 'upside-down')
...
function setupGeocoder(map) {
    ...
    inputEl.addEventListener('input', () => {
        ...
        searchTimeout = setTimeout(async () => {
            try {
                const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=6&accept-language=en`, { headers: { 'User-Agent': 'world-upside-down/1.0' } });
          ...
          `;
                    item.addEventListener('click', () => {
                        ...
                        // Fly to location
                        if (bbox) {
                            const sw = [parseFloat(bbox[2]), parseFloat(bbox[0])];
                            const ne = [parseFloat(bbox[3]), parseFloat(bbox[1])];
                            map.fitBounds([sw, ne], {
    ...
    inputEl.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            const first = resultsEl.querySelector('.geocoder-result');
            if (first)
                first.click();
        }
    });
...
function addGraticule(map) {
    ...
    function getIntervals(zoom) {
        if (zoom >= 10)
            return { major: 1, sub: 0.25, labelSpacing: 5 };
        if (zoom >= 7)
            return { major: 2, sub: 0.5, labelSpacing: 10 };
        if (zoom >= 5)
            return { major: 5, sub: 1, labelSpacing: 20 };
        if (zoom >= 3)
            return { major: 10, sub: 2, labelSpacing: 30 };
        if (zoom >= 2)
            return { major: 15, sub: 5, labelSpacing: 45 };
        return { major: 30, sub: 10, labelSpacing: 60 };
    }
    function buildGraticuleGeoJSON(major, sub, labelSpacing) {
        const features = [];
        ...
        for (let lon = -180; lon <= 180; lon += sub) {
            if (Number.isInteger(lon / major) && lon % major === 0)
                continue;
            const coords = [];
            for (let lat = -85; lat <= 85; lat += 2)
                coords.push([lon, lat]);
        ...
        for (let lat = -80; lat <= 80; lat += sub) {
            if (Number.isInteger(lat / major) && lat % major === 0)
                continue;
            const coords = [];
            for (let lon = -180; lon <= 180; lon += 2)
                coords.push([lon, lat]);
    ...
    map.on('zoomend', () => {
        ...
        if (intervals.major !== currentMajor) {
            ...
            if (src)
                src.setData(buildGraticuleGeoJSON(intervals.major, intervals.sub, intervals.labelSpacing));
        }
    });
...
function applyRisoMisregistration(map) {
    ...
    for (const layer of style.layers) {
        if ((layer.id.startsWith('roads_') || layer.id.startsWith('boundaries')) &&
            ...
            const lineWidth = layer.paint?.['line-width'] || 1;
            try {
                map.addLayer({
                    id: `${layer.id}-riso-offset`,
                    type: 'line',
...
async function init() {
    ...
    map.on('load', () => {
        ...
        setTimeout(async () => {
            if (searchHintCancelled || !twEl || !twText)
                return;
            const sequences = [
                [
    ...
    initDymaxion(mapFrame).then(dymaxion => {
        function updateDymaxionTransition() {
            ...
            if (gratLabels)
                gratLabels.style.opacity = String(1 - t);
            if (dymaxionLabel)
                dymaxionLabel.classList.toggle('visible', t > 0.9);
        }
